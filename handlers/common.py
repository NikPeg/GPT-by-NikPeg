...
async def create_user_req(user_id, user_name, request_text, file_paths=None):
    ...
    await bot.send_message(ADMIN_ID, "CURRENT RUN " + current_run)
    if current_run:
        await bot.send_message(ADMIN_ID, "CANCELLING RUN " + current_run)
    ...
    await bot.send_message(ADMIN_ID, "NEW RUN " + run_id)
    await typing()
